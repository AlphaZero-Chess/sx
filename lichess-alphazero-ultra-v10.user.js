...
(function() {
    ...
    class OpeningFilter {
        /**
         * ULTRA v10.1: ULTRA-STRICT opening move validation with safety fallback
         * BLOCKS: Early queen moves, premature rook moves, poor development, unsupported pawns
         * FIXED: Never blocks ALL legal moves - always allows at least one move
         */
        static isValidOpeningMove(board, move, moveHistory) {
            ...
            // ULTRA-CRITICAL v10.1: Block ALL queen moves before move 7 (14 half-moves)
            if (moveCount < 14 && pieceType === 5) {
                ...
                // Count developed pieces (must have 2+ minor pieces out)
                // FIXED v10.1: Reduced from 3 to 2 to be less restrictive
                const developedPieces = this.countDevelopedPieces(board);
                if (developedPieces < 2) {
                    console.log(`⛔ v10.1 BLOCKED early queen: ${moveUCI} (move ${Math.floor(moveCount/2)+1}, ${developedPieces} pieces)`);
                    return false;
                }
            ...
            if (openingRepertoire && openingRepertoire.length > 0) {
                ...
                for (let entry of openingRepertoire) {
                    ...
                    if (foundMove) {
                        ...
                        // Validate with ULTRA-SAFE SEE v10.1
                        const see = SEE.evaluate(board, foundMove);
                        // FIXED v10.1: Allow slightly losing book moves (-50 threshold)
                        if (see < -50) {
                            console.log(`⛔ v10.1 Book move ${entry.move} filtered by SEE: ${see}`);
                            continue;
                        }
    ...
    class UltraSearchEngine {
        ...
        search(board, maxDepth, timeLimit, moveHistory = []) {
            ...
            if (bestMove) {
                ...
                console.log(`⚡ v10.1 PERFECTION: ${moveUCI} (depth ${this.currentDepth}, ${this.nodes} nodes, TT: ${this.ttHits}, Q: ${this.quiescenceDepth})`);
            } else {
                // FIXED v10.1: Ultimate safety fallback - return any legal move to prevent freeze
                const emergencyMoves = MoveGenerator.generate(board);
                if (emergencyMoves.length > 0) {
                    console.log(`⚠️ v10.1 EMERGENCY: No best move found, playing random legal move to prevent freeze`);
                    bestMove = emergencyMoves[0];
                }
            }
        ...
        alphaBeta(board, depth, alpha, beta, ply, isRoot = false) {
            ...
            let moves = this.generateOrderedMoves(board, ply);
            
            // FIXED v10.1: Safety fallback - if filtering resulted in no moves, use all legal moves
            if (moves.length === 0) {
                const allLegalMoves = MoveGenerator.generate(board);
                if (allLegalMoves.length === 0) {
                    // True checkmate/stalemate
                    return -MATE_SCORE + (this.currentDepth - depth);
                }
                // Filtering was too aggressive - use all legal moves instead
                console.log(`⚠️ v10.1 SAFETY: Filtering too strict, using ${allLegalMoves.length} unfiltered moves`);
                moves = allLegalMoves;
            }
        ...
        generateOrderedMoves(board, ply) {
            ...
            for (let move of moves) {
                ...
                if (isCapture) {
                    ...
                    // FIXED v10.1: Never skip moves entirely - just give very low score
                    // This prevents returning empty move list in difficult positions
                    if (see < -50) {
                        score += see - 3000; // Very low score, but still included
                    } else {
                        score += see + 2000;
                    }
                } else {
                    // Non-capture move baseline
                    score += 0;
                }
