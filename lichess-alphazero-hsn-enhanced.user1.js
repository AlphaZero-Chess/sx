...
(function() {
    ...
    const LichessManager = {
            ...
            window.WebSocket = new Proxy(OriginalWebSocket, {
                construct(target, args) {
                    ...
                    ws.addEventListener('close', () => {
                        Logger.error('⚠️ WebSocket closed - waiting for reconnection');
                        // Reset processing state when connection lost
                    ...
                    ws.addEventListener('error', (err) => {
                        Logger.error('⚠️ WebSocket error detected');
                        STATE.stats.errors++;
                    });
        ...
        handleGameState(message) {
            ...
            if (STATE.processingMove) {
                ...
                if (stuckTime > 10000) {
                    // Stuck for more than 10 seconds - force recovery
                    Logger.error(`⚠️ Force recovery - stuck for ${(stuckTime/1000).toFixed(1)}s`);
            ...
            // Set safety timeout (CONFIG.movetime + 2 seconds buffer)
            STATE.moveTimeout = setTimeout(() => {
                if (STATE.processingMove) {
                    Logger.error('⚠️ Move generation timeout - resetting');
                    STATE.processingMove = false;
                    STATE.stats.errors++;
                    STATE.stats.recoveries++;
                }
            ...
            setTimeout(() => {
                try {
                    ...
                    // Clear timeout if we got a move
                    if (STATE.moveTimeout) {
                        clearTimeout(STATE.moveTimeout);
                        STATE.moveTimeout = null;
                    }
        ...
        sendMove(move) {
            // Clear timeout when sending move
            if (STATE.moveTimeout) {
                clearTimeout(STATE.moveTimeout);
                STATE.moveTimeout = null;
            }
            ...
            if (!STATE.webSocket || STATE.webSocket.readyState !== WebSocket.OPEN) {
                Logger.error('WebSocket not ready - attempting recovery');
                STATE.processingMove = false;
                STATE.stats.errors++;
                return;
            }
        ...
        forceReset() {
            ...
            Logger.success('✅ Engine state reset - ready for next move');
        }
    };
    ...
    function initialize() {
        ...
        console.log('%c🛡️ Auto-recovery enabled: 10s timeout, watchdog active', 'color: #4CAF50; font-size: 12px;');

        STATE.engine = new ChessEngine();
        Logger.success('⚡ HSN Engine initialized - Magnus Carlsen style active');

        LichessManager.install();
        Logger.success('⚡ HSN v9.0 activated - Human-like play at 2200 Elo');

        // Start watchdog timer to detect and fix stuck states
        STATE.watchdogInterval = setInterval(() => {
            if (STATE.processingMove) {
                const elapsed = Date.now() - STATE.processingStartTime;
                if (elapsed > 15000) {
                    // Critical: stuck for 15+ seconds
                    Logger.error(`⚠️ WATCHDOG: Engine stuck for ${(elapsed/1000).toFixed(1)}s - forcing reset`);
                    LichessManager.forceReset();
                    STATE.stats.recoveries++;
                }
            }
        ...
        Logger.success('🛡️ Watchdog timer started - auto-recovery enabled');
    }
    ...
    window.AlphaZeroElite = {
        ...
        /**
         * Force reset if engine is stuck
         */
        forceReset() {
            LichessManager.forceReset();
        ...
        status() {
            ...
            if (STATE.processingMove) {
                ...
                status.status = elapsed > 10000 ? '⚠️ STUCK - use forceReset()' : '✅ Processing';
            } else {
                status.status = '✅ Ready';
            }
